...
import {
  deleteContaPagar,
  marcarPagoContaPagar,
} from '@/lib/actions/contas-pagar';
import { fmtBRL, fmtDate } from '@/lib/utils';
import type { ContaPagar, CpStatus, Empreendimento, Empresa } from '@/types/db';
...
export function ContasPagarClient({
  ...
  const [filterStatus, setFilterStatus] = useState<CpStatus | 'TODOS'>('TODOS');
  ...
  const columns: Column<ContaPagar>[] = [
    ...
    },
    {
      ...
      cell: (r) => (
        <div>
          ...
          {r.numero_documento && (
            <div className="text-xs text-text-dim">{r.numero_documento}</div>
          )}
        </div>
      ),
      searchAccessor: (r) => `${r.descricao} ${r.numero_documento ?? ''}`,
    },
    {
      key: 'obra',
    ...
    },
    {
      key: 'empresa',
      header: 'Fornecedor',
      cell: (r) => (r.empresa_id ? empresaNomes[r.empresa_id] ?? '—' : '—'),
    },
    {
      key: 'etapa',
      header: 'Etapa',
      cell: (r) => r.etapa_eap?.replaceAll('_', ' ') ?? '—',
    },
    {
      key: 'valor',
      header: 'Valor',
  ...
  return (
    <>
      <PageHeader
        ...
        description="Materiais, mão-de-obra, projetos, taxas e demais obrigações."
        actions={
          <button
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        <select
          ...
          value={filterStatus}
          onChange={(e) => setFilterStatus(e.target.value as CpStatus | 'TODOS')}
        >
      ...
      <DataTable
        ...
        searchPlaceholder="Buscar por descrição, doc…"
      />
